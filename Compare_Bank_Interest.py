...
n1=int(input("Number of slabs of interest rates for a given period by bank A "))
a=[input().split() for _ in range(n1)]
n2=int(input("Number of slabs of interest rates for a given period by bank B "))
b=[input().split() for _ in range(n2)]
for i in range(n1):
    for j in range(n2):
        try:
            a[i][j]=float(a[i][j])
